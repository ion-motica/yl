...
(function (global) {
  ...
  global.QuizRegistry.register({
    ...
    create() {
      ...
      // standard-optiuni-cp.md). Acopera Grila/Pozitie coloane/Numerotare/
      // Bara mere/Lift/FOV/Dara — TOATE campurile simple (get/set). Sectiunea
      // "Culori" (mai jos, in appendRigleTabla110ControlPanel) ramane cod
      // separat, NEMIGRAT: are stare proprie (elementul curent selectat),
      // actiuni compuse (salveaza/aplica un SET de 5 culori deodata) si o
      // relatie master-detail intre selector si color-picker — nu se
      // incadreaza curat in formatul "camp = {tip, get, set}". Ramane o
      // intrebare arhitecturala deschisa (cum tratam sisteme CP compuse,
      // cu stare si actiuni proprii, nu doar optiuni simple), pentru mai
      // tarziu — nu blocheaza restul migrarii.
      function campurileCP(rerandeaza) {
        return [
          ...
          { cheie: "liftPornire", tip: "enum", stilAfisare: "radio", eticheta: "Comportament initial lift:",
            optiuni: [
              { valoare: "coloana2", text: "Întotdeauna pe coloana 2" },
              { valoare: "intreColoane", text: "Între 2 coloane" },
            ],
            get: getLiftPornire,
            set: (v) => { global.LayoutConfig?.set(LIFT_PORNIRE_KEY, v); mounted?.setLift({ pornire: v }); } },
          { cheie: "liftScala", tip: "enum", stilAfisare: "radio", eticheta: "Dimensiune initiala lift:",
            optiuni: [
              { valoare: 1, text: "Normala" },
              { valoare: 2, text: "Dubla" },
            ],
            get: getLiftScala,
            set: (v) => { global.LayoutConfig?.set(LIFT_SCALA_KEY, v); mounted?.setLift({ scalaInitiala: v }); } },
