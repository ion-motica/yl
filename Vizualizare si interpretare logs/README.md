...
Un **fixture** este un set mic și controlat de date sintetice folosit pentru demonstrație și teste repetabile. Butonul **Folosește fixture-ul inclus** încarcă acest set local; nu citește automat jurnalul real al elevului. **Fluență standard** este momentan un preset provizoriu de filtrare și praguri, nu o evaluare definitivă.

Interfața nu presupune în cod dimensiunea unei table. Catalogul declară explicit `randuri`, `coloane` și fiecare celulă `{ fact_id, rand, coloana, eticheta }`; motorul transmite coordonatele, iar vizualizarea le așază fără să parseze identificatorul sau textul exercițiului. Orice fact catalogat pentru acea tablă trebuie să indice un `cell_id` existent; o mapare greșită oprește raportul în loc să piardă întrebări în tăcere. Același contract a fost verificat automat pentru tabla extinsă, orientată ca 20 de rânduri × 10 coloane. Catalogul real pentru acea tablă nu este încă inclus în fixture și nu este inventat de interfață.

Presetul implicit `stare_generala_demo_v1` rămâne demonstrația compactă cu șase facts. Presetul `stare_tabla_inmultirii_1_10_v1` folosește catalogul complet cu 100 de poziții 1–10 × 1–10. Logurile nu sunt extinse artificial: celulele fără observații sunt materializate explicit cu `n_intrebari: 0`, starea și suficiența `netestat`.
